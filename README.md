...
```json
{
  ...
  "containers": [
    { "name": "d455ff67a2b2", "owner": "SPY_Con", "status": "active|idle|stopped", "cpu": 0, "mem": 0, "gpu": 0 }
  ],
  "hourStats": [0,0,0,...],
...
| 4 個 Metric 卡片 | 目前在線人數、今日登入次數、活躍容器數、本週登入次數 |
...
| 容器狀態卡片 | 每個容器的名稱、擁有者、active/idle/stopped 狀態 |
...
| 項目 | 限制 |
|------|------|
| 容器 CPU/記憶體 | 目前顯示 0%，需額外串接 `docker stats` |
| 本週總工時 | 因無精確連線時長，目前為靜態示範值 |
...
- **`docker stats` 串接**：定期抓取容器 CPU/記憶體使用率，讓容器卡片顯示真實數值
