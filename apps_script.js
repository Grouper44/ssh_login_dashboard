...
var TELEGRAM_TOKEN = "YOUR_TELEGRAM_BOT_TOKEN";
...
function doPost(e) {
  ...
  // ── 資源統計（docker stats + nvidia-smi）────────────
  if (action === "stats") {
    try {
      var body = JSON.parse(e.postData.contents);
      var payload = JSON.stringify({
        gpu: Number(body.gpu) || 0,
        containers: body.containers || [],
        time: timestamp
      });
      PropertiesService.getScriptProperties().setProperty("latestStats", payload);
    } catch(err) {}
    return ContentService.createTextOutput("OK");
  }
...
function getDataPayload() {
  ...
  // ── 讀取最新一筆 stats ──
  var gpuUtil = 0;
  var statsMap = {};  // name -> { cpu, mem }
  try {
    var raw = PropertiesService.getScriptProperties().getProperty("latestStats");
    if (raw) {
      var stats = JSON.parse(raw);
      gpuUtil = Number(stats.gpu) || 0;
      (stats.containers || []).forEach(function(c) {
        statsMap[c.name] = { cpu: Number(c.cpu) || 0, mem: Number(c.mem) || 0 };
      });
    }
  } catch(e) {}
  ...
  containers = containers.map(function(c) {
    ...
    return {
      ...
      owner:  c.owner,
      status: c.status,
      cpu:    s.cpu !== undefined ? s.cpu : 0,
      mem:    s.mem !== undefined ? s.mem : 0,
      gpu:    gpuUtil
    };
  });
